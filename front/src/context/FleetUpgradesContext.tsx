...
interface FleetUpgradesContextValue {
  catalog: GemUpgradeDef[]
  owned: Set<string>
  /** Not cumulative — only your single highest-tier owned upgrade applies, as a flat multiplier on the whole fleet's output. */
  bestOwned: GemUpgradeDef | null
  buyingId: string | null
  buy: (upgrade: GemUpgradeDef) => Promise<{ ok: boolean; error?: string }>
}

const FleetUpgradesContext = createContext<FleetUpgradesContextValue | null>(null)

// Same sequential-tier ladder as UpgradesContext (clicks) — just spends
// gems instead. No RevenueCat involved.
export function FleetUpgradesProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    fetch(`${API_URL}/api/fleet-upgrades`)
      .then((r) => r.json())
      .then(setCatalog)
  ...
  const buy = useCallback(
    async (upgrade: GemUpgradeDef) => {
      ...
      try {
        const token = await getToken()
        const res = await fetch(`${API_URL}/api/fleet-upgrades/buy`, {
          method: 'POST',
  ...
  return <FleetUpgradesContext.Provider value={value}>{children}</FleetUpgradesContext.Provider>
}

export function useFleetUpgradesContext() {
  const ctx = useContext(FleetUpgradesContext)
  if (!ctx) throw new Error('useFleetUpgradesContext must be used within a FleetUpgradesProvider')
  return ctx
}
